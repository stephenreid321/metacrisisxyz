...
class Concept
  ...
  def self.discover
    ...
    phrase2_frequency = words.each_cons(2).reject { |a, b| stops.include?(a) || stops.include?(b) || stops.include?("#{a} #{b}") || (a.length < 4 || b.length < 4) || a.start_with?('[[') || a.end_with?(']]') || b.start_with?('[[') || b.end_with?(']]') }.each_with_object(Hash.new(0)) { |word, counts| counts[word.join(' ')] += 1 }
